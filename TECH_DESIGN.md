...
## 9. 踩坑记录（真实 bug 也是面试素材）

| Bug | 现象 | 根因 | 教训 |
|---|---|---|---|
| 建库失败 1049 | 启动时 CREATE DATABASE 报 Unknown database | `database or settings.mysql_db` 把 `None`（"只连服务器"语义）当 falsy 回退成库名，导致"连不存在的库去建库" | `None` 是合法语义值，别用 `or` 兜底默认值 |
| 入库失败 1046 | INSERT 报 No database selected | `get_engine()` 忘传库名，引擎无默认库 | 参数默认值语义要清晰，连接必须显式指定库 |
| 沙箱环境 venv 失败 | ensurepip 写临时目录被拒 | 环境策略限制 tempfile 临时目录写入 | 环境问题先看报错堆栈定位到"谁在写哪里" |

---

> 更新记录：v0.1 2026-08-21 初版（覆盖第一、二阶段全部技术方案与面试要点）。
