...
>
> 当前版本：v1.56 ｜ 创建日期：2026-08-20 ｜ 最近更新：2026-09-27
...
- 运行镜像使用 `rag-<服务>:rag-<上游版本>` 的本地项目标签（后端为 `rag-backend:rag-0.1.0`）；首次部署按 `deploy/README.md` 的 `docker tag` 命令给已有上游镜像添加别名，Compose 不拉取不存在的 `rag-*` 仓库；
...
| 2026-09-27 | v1.56 | RAG 后端 tag 改为带应用版本的 `rag-0.1.0`，同步部署说明；`zhangui-deploy/docker-compose.yml` 的五个镜像改用 `zhangui-*` 名称与 `zhangui-<版本>` tag | 按项目归属区分本机 Docker 镜像，避免 RAG 与 Zhangui 服务混淆 |
