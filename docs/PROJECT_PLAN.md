...
>
> 当前版本：v1.31 ｜ 创建日期：2026-08-20 ｜ 最近更新：2026-09-18
...
| 第六阶段 | 工程稳定性优化 | ⑧ | 🟡 进行中 |
| 第十阶段 | Ollama 本地 LLM 与全链路私有化 | ⑨ | 🟡 进行中 |
...
- 上传去重（v1.8）：同一知识库下同名文件拒绝上传，避免重复文件造成检索结果成对重复。**待调整（v1.21 决策，暂未实施）**：改为「显式覆盖」——接口新增 `overwrite` 参数，默认仍拒绝，用户确认后替换（先写新、成功后再删旧）；排在第九阶段之后实施，取舍见 TECH_DESIGN §2.4。

### 8.6 第六阶段：工程稳定性优化（🟡 进行中，执行顺序 ⑧）
...
### 8.10 第十阶段：Ollama 本地 LLM 与全链路私有化（规划，执行顺序 ⑨）
